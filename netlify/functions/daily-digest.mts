...
function scoreArticle(title: string, description: string): "HIGH" | "MEDIUM" | "LOW" {
...
function buildEmailHtml(articles: DigestArticle[]): string {
  ...
  for (const [track, items] of Object.entries(byTrack)) {
    ...
    const shown = [...high, ...medium].slice(0, 6);
    ...
    sections += `
      <div style="margin-bottom:32px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;padding-bottom:10px;border-bottom:1px solid #1f1f1f;">
          ...
          <span style="font-size:11px;color:#4b5563;margin-left:auto;">${high.length} high · ${medium.length} medium</span>
        </div>
        ${cards}
      </div>`;
  }
...
<body style="margin:0;padding:0;background:#000;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    <div style="margin-bottom:32px;padding-bottom:24px;border-bottom:1px solid #1f1f1f;">
      ...
      <p style="margin:0;font-size:13px;color:#6b7280;">${today} · ${totalHigh} high priority · ${totalMed} medium priority</p>
    </div>
    ${sections}
...
const handler = schedule("0 9 * * *", async () => {
    ...
    FEEDS.map(async (feed) => {
      try {
        ...
        for (const item of (result.items || []).slice(0, 10)) {
          ...
          const priority = scoreArticle(title, description);
  ...
  console.log(`Digest sent to ${toEmail} with ${articles.length} articles`);
  return { statusCode: 200 };
});
